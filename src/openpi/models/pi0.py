...
class Pi0(_model.BaseModel):
    ...
    def sample_actions_rtc(
        ...
        rng: at.KeyArrayLike,
        observation: _model.Observation,
        previous_actions: at.Float[at.Array, "b ah ad"],
        prefix_weights: at.Float[at.Array, "b ah 1"],
        *,
        max_guidance_weight: float | at.Float[at.Array, ""] = 5.0,
        num_steps: int | at.Int[at.Array, ""] = 10,
        noise: at.Float[at.Array, "b ah ad"] | None = None,
    ) -> _model.Actions:
        """Generate an action chunk with guided inference-time RTC.

        ``previous_actions`` and ``prefix_weights`` always have the checkpoint's
        fixed ``[B, H, D]``/``[B, H, 1]`` shapes.  Keeping delay out of Python
        control flow lets a single JIT executable serve changing runtime delays.
        """
        ...
        def step(carry):
            ...
            def endpoint_fn(x):
                v_t = denoise(x, time)
                # This repository integrates from t=1 (noise) to t=0 (action).
                return x - time * v_t, v_t

            x0_hat, pullback, v_t = jax.vjp(endpoint_fn, x_t, has_aux=True)
            error = jax.lax.stop_gradient((previous_actions - x0_hat) * prefix_weights)
            correction = pullback(error)[0]

            tau = 1.0 - time
            denominator = jnp.maximum(time * tau, jnp.finfo(x_t.dtype).eps)
            raw_guidance_weight = (jnp.square(time) + jnp.square(tau)) / denominator
            guidance_weight = jnp.minimum(jnp.asarray(max_guidance_weight, dtype=x_t.dtype), raw_guidance_weight)
            v_rtc = v_t - guidance_weight * correction
            return x_t + dt * v_rtc, time + dt
